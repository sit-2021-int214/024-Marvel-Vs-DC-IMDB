...
#### Explan :
```{R}
หาผลรวมคะแนนสูงสุดของทางฝั่ง DC และ Marvel โดยใช้คำสั่ง sum และใส่เงื่อนไขลงไปให้ Category == "DC" , "Marvel " เพื่อเช็คว่ามีทั้งหมดกี่เรื่องที่เข้าเงื่อนไข และ IMDB_Score == max(Marvel_DC$IMDB_Score) พอได้แล้ว เราทำการใช้คำสั่ง เลือกการแสดงผลว่า filter() เพื่อเป็นการกำหนดเงื่อนไขลงไป อันดับสุดท้ายการแสดงผลจะให้มีแค่ Movie_Name,IMDB_Score,Category เลยโดยใช้คำสั่ง select()
```
### 2. ปี 2021 มีภาพยนต์ทั้งหมดกี่เรื่อง หาค่าเฉลี่ยของเวลาฉายทั้งหมดของปี 2021 
```{R}
...
#### Explan :
```{R}
โจทย์ถามว่า ปี 2021 มีภาพยนต์ทั้งหมดกี่เรื่อง เราเลยใช้คำสั่ง sum() เพื่อหาดูภาพยนต์ทั้งหมดภายในปี 2021 และการหาค่าเฉลี่ยเราใช้คำสั่ง mean("โดยกำหนดเงื่อนไขลไปว่า Marvel_DC$Year_Started == 2021")
```
### 3. ประเภทของภาพยนต์มีอะไรบ้าง และ แต่ละประเภทมีเท่าไหร่
```{R}
...
#### Explan :
```{R}
โจทย์ถามหา ประเภทของภาพยนต์มีอะไรบ้าง และ แต่ละประเภทมีเท่าไหร่ ตอน clean เราได้มีการ แยกข้อมูลในส่วนของ Genre ออกมาเพื่อทำให้ข้อมูลนั้นชัดเจนมากยิ่งขึ้น เราได้ใช้คำสั่ง count(Genre) เพื่อนับจำนวนประเภทของภาพยนต์ทั้งหมด และใช้คำสั่ง unnest(Genre) ในการเรียกดูประเภทของภาพยนต์ทั้งหมด
```
### 4.) ภาพยนต์เรื่องไหนบ้างที่มีเนื้อหาไม่เหมาะสมกับเด็กและเยาวชนที่อายุต่ำกว่า 13 ปี ฝั่ง DC
```{R}
...
#### Explan :
```{R}
อันดับแรก เราได้มีการจัดกลุ่มข้อมูลของ Rating โดยใช้คำสั่ง factor() และ assign ค่าลงไปใน 1 ตัวแปร และนำตัวแปรที่เรากำหนดมา มาใส่เงื่อนไขโดยใช้ filter(RatingMovie == "PG-13" & Marvel_DC$Category == "DC") อันดับสุดท้ายการแสดงผลจะให้มีแค่ Movie_Name ,Rating ,Category เลยใช้คำสั่ง select()
```
### 5.) หาค่าโหวตต่ำสุด และ ผลรวมของทั้ง 2 ค่าย แล้วนำค่าผลรวมของทั้ง 2 ค่ายมาเปรียบเทียบกันว่าค่ายไหนมีผลโหวตมากกว่ากัน
```{R}
#MIN DC
...
#### Explan :
```{R}
เราได้มีการจัดกลุ่มข้อมูลโดยใช้คำสั่ง fartor() โดยจัดกลุ่มของ Votes และมีการกำหนดเงื่อนไขว่า  Marvel_DC$Votes == min(Marvel_DC$Votes) & Marvel_DC$Category == "DC" เพื่อหาค่าต่ำสุดของ DC และกำหนดค่าลงไปตัวแปรที่ชื่อว่า MinDC และเลือกแสดงผลข้อมูลที่ไม่ซ้ำจาก MinDC โดยใช้คำสั่ง distinct() ในส่วนของ Marvel พวกเราได้ใช้คำสั่ง min() ในการหาแต่ขึ้นเป็น 0 rows พวกเราเลยแก้ปัญหาโดยการ นั่งหาค่าต่ำที่สุดโดยการไล่เลข และ ตรวจดูจากการ glimpse() ข้อมูลออกมาดู แล้วพบว่า min คือ 17 เลยกำหนดเงื่อนไขเป็น Votes == 17 & Category == "Marvel"
...
#### Explan :
```{R}
จากโจทย์ต้องการหา  มีจำนวนภาพยนต์กี่เรื่องที่มีเวลาฉายต่ำกว่า 100 นาที ทั้งฝั่งของ Marvel และ DC เลยใช้คำสั่ง filter() โดยมีเงื่อนไขว่า Marvel_DC$RunTime < 100 แล้ว assign ลง 1 ตัวแปรนั่นก็คือ lessonehundred และเรานำตัวแรปที่เรากำหนดนนั้นมานับว่ามีทั้งหมดกี่เรื่องโดยใช้คำสั่ง count() จากโจทย์ให้ยกตัวอย่างมา 6 เรื่อง เราเลยใช้คำสั่ง head() เป็นตัวช่วยในการแสดงผล
```
